...
namespace ImportShopOffers
{
    ...
    public class OfferCategories
    {
        ...
        public void Load(XElement categories)
        {
            ...
            foreach (var categoryElement in categories.Elements().Where(item => item.Attribute("parentId") == null))
            {
                var offcat = new OfferCategory()
                                {
                                    Id = int.Parse(categoryElement.Attribute("id").Value),
            ...
            while (count > 0)
            {
                ...
                foreach (var categoryElement in categories.Elements().Where(item => item.Attribute("parentId") != null))
                {
                    var parentId = int.Parse(categoryElement.Attribute("parentId").Value);
